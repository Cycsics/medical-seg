...
from .Addmodules.temporal_memory import TemporalMemoryBlock
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        # Build strides
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect, DSDConvSegment)):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            s = 256  # 2x min stride
            ...
            def _forward(x):
                ...
                return self.forward(x)[0] if isinstance(m, (Segment, YOLOESegment, Pose, OBB, DSDConvSegment)) else self.forward(x)
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            {Detect, WorldDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, ImagePoolingAttn, v10Detect, DSDConvSegment}
        ):
            args.append([ch[x] for x in f])
            if m in (Segment, DSDConvSegment, YOLOESegment):
...
def guess_model_task(model):
    ...
    if isinstance(model, torch.nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Segment, YOLOESegment, DSDConvSegment)):
                return "segment"
